...
def run_module():
    ...
    if module.params['state'] == 'absent':
        if etcdctl.etcd_user_check(module.params['name']):
            if not etcdctl.etcd_user_delete(module.params['name']):
                module.fail_json(msg='Cannot delete user %s' % (module.params['name']))
            result['changed'] = True
        else:
        ...
        if etcdctl.etcd_user_check(module.params['name']):
            ...
            roles= etcdctl.etcd_user_role_list(module.params['name'])
            if roles !=module.params['role']:
            #user exist with incorect roles
                etcdctl.etcd_user_grant_role(module.params['name'], module.params['role'])
                result['changed'] = True
        else:
            #create user
            etcdctl.etcd_user_add(module.params['name'], 'none')
            #add roles
